-- @description IFLS_Toolbar_Suite: IFLS BeatControlCenter (migrated hub)
-- @version 1.0.2
-- @author IFLS_Toolbar_Suite
-- @about Migrated from legacy IFLS hub; adjusted internal paths to Scripts/IFLS_Toolbar_Suite.

-- IFLS_Toolbar_Suite (migrated) - IFLS_BeatControlCenter_ImGui.lua
local r = reaper
local resource_path = r.GetResourcePath()
local ig = r.ImGui
if not ig then
  local ok_imgui, mod = pcall(require, "imgui")
  if ok_imgui and type(mod) == "table" then
    ig = mod
    -- optional convenience: expose as reaper.ImGui table for code expecting it
    r.ImGui = r.ImGui or ig
  else
    error("module 'imgui' not found. Make sure Scripts/IFLS_Toolbar_Suite/lib/imgui.lua exists and ReaImGui is installed.\n" .. tostring(mod))
end
end
...
local suite_root = (_this_dir() .. '../'):gsub('\\\\','/')
local core_path   = suite_root .. 'Core/'
local domain_path = suite_root .. 'Domain/'
